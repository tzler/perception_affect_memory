...
generalization_data = data %>% 
  ...
  mutate(association = ifelse(key_press==positive_key, 'positive', 
                              ifelse(key_press==negative_key, 'negative', NaN)))
...
hypothesis_space = generalization_data %>% 
  ...
  select(valence, distance, match, subject, correct, association)
...
hypothesis_space %>% 
  ...
  ggplot(aes(x=distance, y=p_association, fill=valence)) + 
    geom_line(aes(color=valence), size=1) + 
    geom_ribbon(aes(ymin=y_lower, ymax=y_upper), alpha=.2) + 
    ggtitle("'Replication' of main finding (n=20)")
...
hypothesis_space %>% 
  ...
  #filter(subject==attention_check$subject[1]) %>% 
  group_by( valence, distance) %>% 
  summarise(p_association = mean(match, na.rm = TRUE ), 
            sem = sd(match, na.rm = TRUE)/sqrt(length(match)),
            y_lower = p_association-sem, 
  ...
  ggplot(aes(x=distance, y=p_association, fill=valence)) + 
    geom_line(aes(color=valence), size=1) + 
    geom_ribbon(aes(ymin=y_lower, ymax=y_upper), alpha=.4) + 
...
hypothesis_space %>% 
  filter(subject==attention_check$subject[9]) %>% 
  group_by( valence, distance) %>% 
  summarise(p_association = mean(match, na.rm = TRUE ), 
            sem = sd(match, na.rm = TRUE)/sqrt(length(match)),
            y_lower = p_association-sem, 
            y_upper = p_association+sem) %>% 
  ggplot(aes(x=distance, y=p_association, fill=valence)) + 
    geom_ribbon(aes(ymin=y_lower, ymax=y_upper), alpha=.5) +
    ggtitle("Only subjects who passed a post-mortem attention check")
...
attention_check_hypothesis = filter(hypothesis_space, subject %in% attention_check$subject)
summary(lm('match ~ valence * distance', attention_check_hypothesis))
...
hypothesis_space %>% 
  filter(correct==0) %>% 
  group_by( valence, distance) %>% 
  summarise(p_association = sum(association=='negative', na.rm = TRUE)) %>% 
  ggplot(aes(x=distance, y=p_association, color=valence)) + geom_line()  + 
  ylim(0, 110)
...
hypothesis_space %>% 
  filter(correct==0) %>% 
  group_by( valence, distance) %>% 
  summarise(p_association = sum(association=='positive', na.rm = TRUE)) %>% 
  ggplot(aes(x=distance, y=p_association, color=valence)) + geom_line() + 
  ylim(0, 110)
...
data %>%  
  mutate(log_distance = as.factor(round(log(abs(distance+1))))) %>% 
  filter(stage=='generalization' & valence=='positive '  & subject %in% attention_check$subject) %>% 
  group_by(log_distance) %>% 
  ggplot(aes(x=log_distance, y=rt, color=log_distance)) + 
  geom_violin()  
